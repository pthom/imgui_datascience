...
class ImageWithZoomInfo():
    ...
    def is_not_full_view(self):
        fullview_affine_transform = ZoomInfo.make_full_view(SizePixel.from_image(self.image), self.current_viewport_size()).affine_transform
...
def color_msg(color):
    ...
    if isinstance(color, np.uint8):
        color_msg = "{0}".format(color)
    elif isinstance(color, np.float32):
        color_msg = "{0:.3f}".format(color)
        ...
        if len(color) == 3:
            ...
            imgui.same_line()
            color_msg = "RGB({0},{1},{2})".format(bgr[2], bgr[1], bgr[0])
        elif len(color) == 4:
            bgra = color
...
def image_explorer_impl(im, title =""):
    ...
    if not im.hide_buttons and not mouse_location is None:
        mouse_drag_button = 0
        ...
        if is_mouse_dragging:
            drag_delta_delta = imgui.Vec2(drag_delta.x - im.zoom_info.last_delta.x, drag_delta.y - im.zoom_info.last_delta.y)
    ...
    if not im.hide_buttons:
        ...
        if not im.show_adjustements:
            if imgui.small_button(make_unique_label_im("Adjust")):
                im.show_adjustements = True

        # adjustments
        if im.show_adjustements:
            imgui.new_line()
            imgui.text("Adjust:"); imgui.same_line()
            ...
            if imgui.small_button(make_unique_label_im("hide adjust")):
                im.show_adjustements = False

        # Show image info
        # viewport_msg = "Viewport({0}, {1})".format(im.current_viewport_size().height, im.current_viewport_size().width)
...
def image_explorer_autostore_zoominfo(image, viewport_size, title, zoom_key, image_adjustments, hide_buttons):
    ...
    if not zoom_key in statics.all_zoom_info:
        statics.all_zoom_info[zoom_key] = ZoomInfo.make_full_view(SizePixel.from_image(image), viewport_size)

    flag_need_update = False
    if not image_key in statics.all_ImageWithZoomInfo :
        flag_need_update = True
    elif id(statics.all_ImageWithZoomInfo[image_key].image) != id(image):
        flag_need_update = True

    if flag_need_update:
        statics.all_ImageWithZoomInfo[image_key] = ImageWithZoomInfo(image, viewport_size, statics.all_zoom_info[zoom_key], hide_buttons = hide_buttons, image_adjustments=image_adjustments)
